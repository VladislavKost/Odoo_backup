...
def parameters(config_path):
    ...
    logging.basicConfig(level=logging.INFO, filename='UploadInformation.log', filemode="w",
                        format='%(asctime)s %(levelname)s:%(message)s')
    try:
        common = xmlrpc.client.ServerProxy('{}/xmlrpc/2/common'.format(params['url']))
        uid = common.authenticate(params['db'], params['username'], params['password'], {})
        models = xmlrpc.client.ServerProxy('{}/xmlrpc/2/object'.format(params['url']))
        return params, uid, models
    except xmlrpc.client.Fault:
        logging.info(f'Database connection error. Incorrect database name. Current database name is "{params["db"]}"')
...
class Asynchron:
    ...
    async def request_entity(self, session, url):
        ...
        while True:
            try:
                ...
                logging.info(f'Data acquisition error. Repeated attempt to receive data at {url}')
    ...
    async def request_all(self, urls, type='', photo=False):
        ...
        async with aiohttp.ClientSession() as session:
            if photo:
                json_list = await tqdm_asyncio.gather(*[self.request_photo(session, url) for url in urls],
...
class Planets:
    ...
    def generate_planet_info(self, json_list):
        ...
        for i in range(len(json_list)):
            ...
            diameter, rotation_period, orbital_period, population = [value if (value != 'unknown' and value != '0') else '' for
                                                                     value in [diameter, rotation_period, orbital_period, population]]
            planet_dict.update({planet_id: {'name': name, 'diameter': diameter, 'rotation_period': rotation_period,
                                            'orbital_period': orbital_period, 'population': population}})
        return planet_dict
...
class Odoo:
    ...
    def check_entity_in_odoo(self, params, base_model, uid, models, entity_dict):
        ...
        try:
            result = models.execute_kw(params["db"], uid, params["password"], base_model, 'search_read',
                                       [[['name', '!=', '']]], {'fields': ['name']})
            new_dict = {}
            del_list = []
            ids_entity_dict = {}
            ...
            print('Ошибка подключения к БД. Неверное имя пользователя, пароль или модель')
            sys.exit(1)

    def upload_entity_info_into_oddo(self, params, base_model, uid, models, entity_dict, ids_entity_dict, type):
        ...
        entity_odoo_id = models.execute_kw(params["db"], uid, params["password"], base_model, 'create',
...
if __name__ == "__main__":
    ...
    odoo = Odoo()
    planet_dict, ids_planets_dict = odoo.check_entity_in_odoo(params, params["planets_model"], uid, models, planet_dict)
    ids_planets_dict = odoo.upload_entity_info_into_oddo(params, params["planets_model"], uid, models, planet_dict, ids_planets_dict, 'planet')
    ...
    urls_to_request = get_photo.generate_photo_urls(params.get('image_url'), len(characters_dict) + 1)
    ...
    changed_characters_dict = character.upgrage_character_photo(characters_dict, image_dict)

    # Загружаем героев в Odoo
    characters_dict, ids_character_dict = odoo.check_entity_in_odoo(params, params["characters_model"], uid, models, changed_characters_dict)
